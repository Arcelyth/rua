...
impl Sprite {
    ...
    fn get_frame(&self, index: u32) -> RuaImage {
        ...
        let (row, col) = match &self.direc {
            Some(d) => match d {
                Direction::Column(c) => (index / cols, *c),
                Direction::Row(r) => (*r, index % cols),
            },
            None => (index / cols, index % cols),
        };
